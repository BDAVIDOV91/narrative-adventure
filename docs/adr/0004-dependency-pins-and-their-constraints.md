...
| `happy-dom`              | 20.14.0 | —                  | The vitest environment, chosen over `jsdom`: 7 installed packages against jsdom's ~30, and it supplies the only browser surfaces this suite needs — `localStorage`, `requestAnimationFrame`, `devicePixelRatio`, a `<canvas>` element. Rule 9 budgets ~1.9 GB free RAM, so the lighter runner is the one that still runs with a dev server and a browser open. devDependency only; nothing reaches the bundle. |
