...
| "`CLAUDE.md` is group-writable (0o664)" (MEDIUM)                                     | **Accepted risk.** The group is `technojihad` — the owner's own private group, no other members, standard Ubuntu `USERGROUPS` umask. The exploit requires a second account in that group, which does not exist. Note the tool flags `CLAUDE.md` but not `settings.json`, which carries identical permissions. Revisit if this repository is ever checked out on a shared machine. |
